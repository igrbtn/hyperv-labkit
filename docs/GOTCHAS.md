...
## Self-driving гость: порядок и повторы

- **У задачи должен быть повторяющийся триггер.** Только `AtStartup` + `Once`
  недостаточно: гость, чья зависимость ещё не готова (DC промоутится, DNS
  молчит), откатывает состояние - и больше никогда не запускается без ребута.
  Ставить `-RepetitionInterval (New-TimeSpan -Minutes 5)`; state-машина сама
  выключит задачу по достижении `DONE-*`.
- **Настройка сети - жёсткий гейт, а не best-effort.** Если адаптера в `Up` нет,
  выходить со статусом `no-nic`, а не идти дальше: промо леса без сети даёт
  сломанный домен, который проще снести, чем чинить. То же для join.
- **Зависимости проверять фактом, а не таймером**: не `Start-Sleep`, а цикл
  `until` по `Resolve-DnsName`, `Get-ADDomain`, `Get-VirtualDisk` и т.п.
  (приём из MSLab, экономит часы отладки).
